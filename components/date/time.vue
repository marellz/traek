        ...
        <div class="space-y-1 border-r border-r-slate-200 pr-4">
          <div v-for="t in _times" :key="t.time">
            ...
            <label
              :for="timeId + t.time"
              :value="t.time"
              class="hover:bg-primary/10 block rounded px-2 py-0.5 font-medium"
              :class="{
                'bg-primary hover:!bg-primary text-white': t.time === _time.hours,
              }"
            >
              <span>{{ t.label }}</span>
            </label>
      ...
      <div class="space-y-1">
        <div v-for="m in _meridiems" :key="m">
          ...
          <label
            ...
            class="hover:bg-primary/10 block rounded px-2 py-0.5 font-medium"
            :class="{
              'bg-primary hover:!bg-primary text-white': m === _time.meridiem,
            }"
          >
            <span>{{ m }}</span>
          </label>
