<template>
  ...
  >
    <!-- <icon-component /> -->
    <div class="flex-none pl-0 pt-1.5">
      <span class="block rounded-full p-1" :class="themes[variant].color">
        <component :is="icons[variant]" :size="32" :stroke-width="1.5" />
      </span>
    </div>
    <div class="flex-auto space-y-1 pb-2 pt-2">
      <h1 class="font-secondary text-xl font-medium leading-snug" :class="themes[variant].color">
        {{ title }}
      </h1>
      <div>
        <slot />
    ...
    <div v-if="dismissible" class="pt-2">
      <button type="button" class="rounded-full p-2 hover:bg-slate-900/20" @click="dismiss">
        <X />
      </button>
    </div>
  </div>
