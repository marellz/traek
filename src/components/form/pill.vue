<template>
  <div>
    <input type="checkbox" :name class="absolute h-0 w-0 -z-10" :id="`tag-${id}`" :value v-model="model" />
    <label :for="`tag-${id}`"
      ...
      :class="{
        '!border-primary bg-primary text-white hover:!border-primary hover:!text-white !px-2':
          checked,
      }">
      <span>
        {{ label }}
      </span>
      <Check :size="16" v-show="checked" :class="{ 'w-0 invisible': !checked }" />
    </label>
  </div>
</template>
