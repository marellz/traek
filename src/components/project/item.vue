<template>
  <router-link class="block" :to="{ name: 'project', params: { id: item.id } }">
    <div class="h-full space-y-2 rounded-xl border border-slate-300 p-4 flex flex-col items-start">
      ...
      <p v-if="item.description" class="line-clamp-2 text-slate-500 text-sm" :title="item.description">{{ item.description }}
      </p>
      <div class="text-xs text-slate-600 flex space-x-1">
        <span>
